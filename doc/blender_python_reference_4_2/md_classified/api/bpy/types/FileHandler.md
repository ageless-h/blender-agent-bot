...
## Basic FileHandler for Operator that imports just one file[#](#basic-filehandler-for-operator-that-imports-just-one-file "Link to this heading")

When creating a `Operator` that imports files, you may want to
add them ‘drag-and-drop’ support, File Handlers helps to define
a set of files extensions ([`FileHandler.bl_file_extensions`](#bpy.types.FileHandler.bl_file_extensions "bpy.types.FileHandler.bl_file_extensions"))
that the `Operator` support and a [`FileHandler.poll_drop`](#bpy.types.FileHandler.poll_drop "bpy.types.FileHandler.poll_drop")
...
## Basic FileHandler for Operator that imports multiple files[#](#basic-filehandler-for-operator-that-imports-multiple-files "Link to this heading")
...
* [Report issue
  on this page](https://projects.blender.org/blender/blender/issues/new?template=.gitea/issue_template/api_docs.yaml&field:body=%2A%2APage+Information%2A%2A%0D%0AFile%3A+%60bpy.types.FileHandler.rst%60%0D%0ABlender+Version%3A+%604.2%60%0D%0A%5BPermanent+Link%5D%28https%3A%2F%2Fdocs.blender.org%2Fapi%2F4.2%2Fbpy.types.FileHandler.html%29%0D%0A%0D%0A%2A%2AShort+description+of+error%2A%2A%0D%0A%5BPlease+fill+out+a+short+description+of+the+error+here%5D%0D%0A)

On this page

* FileHandler(bpy\_struct)
  + [Basic FileHandler for Operator that imports just one file](#basic-filehandler-for-operator-that-imports-just-one-file)
  + [Basic FileHandler for Operator that imports multiple files](#basic-filehandler-for-operator-that-imports-multiple-files)
