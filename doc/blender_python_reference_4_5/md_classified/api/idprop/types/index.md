Contents

Menu

Expand

Light mode

Dark mode

Auto light/dark, in light mode

Auto light/dark, in dark mode

[ ]
[ ]

Hide navigation sidebar

Hide table of contents sidebar

[Skip to content](#furo-main-content)

Toggle site navigation sidebar

[Blender Python API](../../../meta/index.md)

Toggle Light / Dark / Auto color theme

Toggle table of contents sidebar

[![Logo](_static/blender_logo.svg)

Blender Python API](index.md)

Documentation

* [Quickstart](../../../guides/quickstart.md)
* [API Overview](../../../guides/overview.md)
* [API Reference Usage](../../../guides/api_reference.md)
* [Best Practice](../../../guides/best_practice.md)
* [Tips and Tricks](../../../guides/tips_and_tricks.md)
* [Gotchas](../../../guides/gotchas/index.md)[ ]
* [Advanced](../../../guides/advanced/index.md)[ ]
* [Change Log](../../../guides/change_log.md)

Application Modules

* [Context Access (bpy.context)](../../bpy/context/index.md)
* [Data Access (bpy.data)](../../bpy/data/index.md)
* [Message Bus (bpy.msgbus)](../../bpy/msgbus/index.md)
* [Operators (bpy.ops)](../../bpy/ops/index.md)[ ]
* [Types (bpy.types)](../../bpy/types/index.md)[ ]
* [Utilities (bpy.utils)](../../bpy/utils/index.md)[ ]

  Toggle navigation of Utilities (bpy.utils)

  + [bpy.utils submodule (bpy.utils.previews)](../../bpy/utils/previews.md)
  + [bpy.utils submodule (bpy.utils.units)](../../bpy/utils/units.md)
* [Path Utilities (bpy.path)](../../bpy/path/index.md)
* [Application Data (bpy.app)](../../bpy/app/index.md)[ ]

  Toggle navigation of Application Data (bpy.app)

  + [Application Handlers (bpy.app.handlers)](../../bpy/app/handlers.md)
  + [Application Translations (bpy.app.translations)](../../bpy/app/translations.md)
  + [Application Icons (bpy.app.icons)](../../bpy/app/icons.md)
  + [Application Timers (bpy.app.timers)](../../bpy/app/timers.md)
* [Property Definitions (bpy.props)](../../bpy/props/index.md)

Standalone Modules

* [Audio System (aud)](../../aud/index.md)
* [OpenGL Wrapper (bgl)](../../bgl/index.md)
* [Additional Math Functions (bl\_math)](../../bl_math/index.md)
* [Font Drawing (blf)](../../blf/index.md)
* [BMesh Module (bmesh)](../../bmesh/index.md)[ ]

  Toggle navigation of BMesh Module (bmesh)

  + [BMesh Operators (bmesh.ops)](../../bmesh/ops/index.md)
  + [BMesh Types (bmesh.types)](../../bmesh/types/index.md)
  + [BMesh Utilities (bmesh.utils)](../../bmesh/utils/index.md)
  + [BMesh Geometry Utilities (bmesh.geometry)](../../bmesh/geometry/index.md)
* [Extra Utilities (bpy\_extras)](../../bpy_extras/index.md)[ ]

  Toggle navigation of Extra Utilities (bpy\_extras)

  + [bpy\_extras submodule (bpy\_extras.anim\_utils)](../../bpy_extras/anim_utils/index.md)
  + [bpy\_extras submodule (bpy\_extras.asset\_utils)](../../bpy_extras/asset_utils/index.md)
  + [bpy\_extras submodule (bpy\_extras.object\_utils)](../../bpy_extras/object_utils/index.md)
  + [bpy\_extras submodule (bpy\_extras.io\_utils)](../../bpy_extras/io_utils/index.md)
  + [bpy\_extras submodule (bpy\_extras.image\_utils)](../../bpy_extras/image_utils/index.md)
  + [bpy\_extras submodule (bpy\_extras.keyconfig\_utils)](../../bpy_extras/keyconfig_utils/index.md)
  + [bpy\_extras submodule (bpy\_extras.mesh\_utils)](../../bpy_extras/mesh_utils/index.md)
  + [bpy\_extras submodule (bpy\_extras.node\_utils)](../../bpy_extras/node_utils/index.md)
  + [bpy\_extras submodule (bpy\_extras.view3d\_utils)](../../bpy_extras/view3d_utils/index.md)
  + [bpy\_extras submodule (bpy\_extras.id\_map\_utils)](../../bpy_extras/id_map_utils/index.md)
* [Freestyle Module (freestyle)](../../freestyle/index.md)[ ]

  Toggle navigation of Freestyle Module (freestyle)

  + [Freestyle Types (freestyle.types)](../../freestyle/types/index.md)
  + [Freestyle Predicates (freestyle.predicates)](../../freestyle/predicates/index.md)
  + [Freestyle Functions (freestyle.functions)](../../freestyle/functions/index.md)
  + [Freestyle Chaining Iterators (freestyle.chainingiterators)](../../freestyle/chainingiterators/index.md)
  + [Freestyle Shaders (freestyle.shaders)](../../freestyle/shaders/index.md)
  + [Freestyle Utilities (freestyle.utils)](../../freestyle/utils/index.md)[ ]

    Toggle navigation of Freestyle Utilities (freestyle.utils)

    - [freestyle.utils submodule (freestyle.utils.ContextFunctions)](../../freestyle/utils/ContextFunctions.md)
* [GPU Module (gpu)](../../gpu/index.md)[ ]

  Toggle navigation of GPU Module (gpu)

  + [GPU Types (gpu.types)](../../gpu/types/index.md)
  + [GPU Matrix Utilities (gpu.matrix)](../../gpu/matrix/index.md)
  + [GPU Select Utilities (gpu.select)](../../gpu/select/index.md)
  + [GPU Shader Utilities (gpu.shader)](../../gpu/shader/index.md)
  + [GPU State Utilities (gpu.state)](../../gpu/state/index.md)
  + [GPU Texture Utilities (gpu.texture)](../../gpu/texture/index.md)
  + [GPU Platform Utilities (gpu.platform)](../../gpu/platform/index.md)
  + [GPU Capabilities Utilities (gpu.capabilities)](../../gpu/capabilities/index.md)
* [GPU Utilities (gpu\_extras)](../../gpu_extras/index.md)[ ]

  Toggle navigation of GPU Utilities (gpu\_extras)

  + [gpu\_extras submodule (gpu\_extras.batch)](../../gpu_extras/batch/index.md)
  + [gpu\_extras submodule (gpu\_extras.presets)](../../gpu_extras/presets/index.md)
* ID Property Access (idprop.types)
* [Image Buffer (imbuf)](../../imbuf/index.md)[ ]

  Toggle navigation of Image Buffer (imbuf)

  + [Image Buffer Types (imbuf.types)](../../imbuf/types/index.md)
* [Math Types & Utilities (mathutils)](../../mathutils/index.md)[ ]

  Toggle navigation of Math Types & Utilities (mathutils)

  + [Geometry Utilities (mathutils.geometry)](../../mathutils/geometry/index.md)
  + [BVHTree Utilities (mathutils.bvhtree)](../../mathutils/bvhtree/index.md)
  + [KDTree Utilities (mathutils.kdtree)](../../mathutils/kdtree/index.md)
  + [Interpolation Utilities (mathutils.interpolate)](../../mathutils/interpolate/index.md)
  + [Noise Utilities (mathutils.noise)](../../mathutils/noise/index.md)

* 4.5

  Versions

  + Loading...

Note

You are not using the most up to date version of the documentation.
 is the newest version.

Back to top

Toggle Light / Dark / Auto color theme

Toggle table of contents sidebar

# ID Property Access (idprop.types)[¶](#module-idprop.types "Link to this heading")

*class* idprop.types.IDPropertyArray[¶](#idprop.types.idprop.types.IDPropertyArray "Link to this definition")
:   to\_list()[¶](#idprop.types.idprop.types.IDPropertyArray.to_list "Link to this definition")
    :   Return the array as a list.

    typecode[¶](#idprop.types.idprop.types.IDPropertyArray.typecode "Link to this definition")
    :   The type of the data in the array {‘f’: float, ‘d’: double, ‘i’: int, ‘b’: bool}.

*class* idprop.types.IDPropertyGroup[¶](#idprop.types.idprop.types.IDPropertyGroup "Link to this definition")
:   clear()[¶](#idprop.types.idprop.types.IDPropertyGroup.clear "Link to this definition")
    :   Clear all members from this group.

    get(*key*, *default=None*)[¶](#idprop.types.idprop.types.IDPropertyGroup.get "Link to this definition")
    :   Return the value for key, if it exists, else default.

    items()[¶](#idprop.types.idprop.types.IDPropertyGroup.items "Link to this definition")
    :   Iterate through the items in the dict; behaves like dictionary method items.

    keys()[¶](#idprop.types.idprop.types.IDPropertyGroup.keys "Link to this definition")
    :   Return the keys associated with this group as a list of strings.

    pop(*key*, *default*)[¶](#idprop.types.idprop.types.IDPropertyGroup.pop "Link to this definition")
    :   Remove an item from the group, returning a Python representation.

        Raises:
        :   **KeyError** – When the item doesn’t exist.

        Parameters:
        :   * **key** (*str*) – Name of item to remove.
            * **default** (*Any*) – Value to return when key isn’t found, otherwise raise an exception.

    to\_dict()[¶](#idprop.types.idprop.types.IDPropertyGroup.to_dict "Link to this definition")
    :   Return a purely Python version of the group.

    update(*other*)[¶](#idprop.types.idprop.types.IDPropertyGroup.update "Link to this definition")
    :   Update key, values.

        Parameters:
        :   **other** (`IDPropertyGroup` | dict[str, Any]) – Updates the values in the group with this.

    values()[¶](#idprop.types.idprop.types.IDPropertyGroup.values "Link to this definition")
    :   Return the values associated with this group.

    name[¶](#idprop.types.idprop.types.IDPropertyGroup.name "Link to this definition")
    :   The name of this Group.

*class* idprop.types.IDPropertyGroupIterItems[¶](#idprop.types.idprop.types.IDPropertyGroupIterItems "Link to this definition")

*class* idprop.types.IDPropertyGroupIterKeys[¶](#idprop.types.idprop.types.IDPropertyGroupIterKeys "Link to this definition")

*class* idprop.types.IDPropertyGroupIterValues[¶](#idprop.types.idprop.types.IDPropertyGroupIterValues "Link to this definition")

*class* idprop.types.IDPropertyGroupViewItems[¶](#idprop.types.idprop.types.IDPropertyGroupViewItems "Link to this definition")

*class* idprop.types.IDPropertyGroupViewKeys[¶](#idprop.types.idprop.types.IDPropertyGroupViewKeys "Link to this definition")

*class* idprop.types.IDPropertyGroupViewValues[¶](#idprop.types.idprop.types.IDPropertyGroupViewValues "Link to this definition")

[Next

Image Buffer (imbuf)](../../imbuf/index.md)
[Previous

gpu\_extras submodule (gpu\_extras.presets)](../../gpu_extras/presets/index.md)

Copyright © Blender Authors

Made with
[Furo](https://github.com/pradyunsg/furo)

* [Report issue
  on this page](https://projects.blender.org/blender/blender/issues/new?template=.gitea/issue_template/api_docs.yaml&field:body=%2A%2APage+Information%2A%2A%0D%0AFile%3A+%60idprop.types.rst%60%0D%0ABlender+Version%3A+%604.5%60%0D%0A%5BPermanent+Link%5D%28https%3A%2F%2Fdocs.blender.org%2Fapi%2F4.5%2Fidprop.types.html%29%0D%0A%0D%0A%2A%2AShort+description+of+error%2A%2A%0D%0A%5BPlease+fill+out+a+short+description+of+the+error+here%5D%0D%0A)

On this page

* ID Property Access (idprop.types)
  + [`idprop.types.IDPropertyArray`](#idprop.types.idprop.types.IDPropertyArray)
    - [`idprop.types.IDPropertyArray.to_list()`](#idprop.types.idprop.types.IDPropertyArray.to_list)
    - [`idprop.types.IDPropertyArray.typecode`](#idprop.types.idprop.types.IDPropertyArray.typecode)
  + [`idprop.types.IDPropertyGroup`](#idprop.types.idprop.types.IDPropertyGroup)
    - [`idprop.types.IDPropertyGroup.clear()`](#idprop.types.idprop.types.IDPropertyGroup.clear)
    - [`idprop.types.IDPropertyGroup.get()`](#idprop.types.idprop.types.IDPropertyGroup.get)
    - [`idprop.types.IDPropertyGroup.items()`](#idprop.types.idprop.types.IDPropertyGroup.items)
    - [`idprop.types.IDPropertyGroup.keys()`](#idprop.types.idprop.types.IDPropertyGroup.keys)
    - [`idprop.types.IDPropertyGroup.pop()`](#idprop.types.idprop.types.IDPropertyGroup.pop)
    - [`idprop.types.IDPropertyGroup.to_dict()`](#idprop.types.idprop.types.IDPropertyGroup.to_dict)
    - [`idprop.types.IDPropertyGroup.update()`](#idprop.types.idprop.types.IDPropertyGroup.update)
    - [`idprop.types.IDPropertyGroup.values()`](#idprop.types.idprop.types.IDPropertyGroup.values)
    - [`idprop.types.IDPropertyGroup.name`](#idprop.types.idprop.types.IDPropertyGroup.name)
  + [`idprop.types.IDPropertyGroupIterItems`](#idprop.types.idprop.types.IDPropertyGroupIterItems)
  + [`idprop.types.IDPropertyGroupIterKeys`](#idprop.types.idprop.types.IDPropertyGroupIterKeys)
  + [`idprop.types.IDPropertyGroupIterValues`](#idprop.types.idprop.types.IDPropertyGroupIterValues)
  + [`idprop.types.IDPropertyGroupViewItems`](#idprop.types.idprop.types.IDPropertyGroupViewItems)
  + [`idprop.types.IDPropertyGroupViewKeys`](#idprop.types.idprop.types.IDPropertyGroupViewKeys)
  + [`idprop.types.IDPropertyGroupViewValues`](#idprop.types.idprop.types.IDPropertyGroupViewValues)