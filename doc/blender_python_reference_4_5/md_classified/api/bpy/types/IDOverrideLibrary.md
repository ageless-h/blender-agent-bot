...
:   Struct gathering all data needed by overridden linked IDs
    ...
    :   Resync the data-block and its sub-hierarchy, or the whole hierarchy if requested
        ...
        :   * **scene** ([`Scene`](Scene.md#bpy.types.Scene "bpy.types.Scene"), (never None)) – The scene to operate in (for contextual things like keeping active object active, ensuring all overridden objects remain instantiated, etc.)
            ...
            * **do\_hierarchy\_enforce** (*boolean**,* *(**optional**)*) – Enforce restoring the dependency hierarchy between data-blocks to match the one from the reference linked hierarchy (WARNING: if some ID pointers have been purposedly overridden, these will be reset to their default value)
...
* [Report issue
  on this page](https://projects.blender.org/blender/blender/issues/new?template=.gitea/issue_template/api_docs.yaml&field:body=%2A%2APage+Information%2A%2A%0D%0AFile%3A+%60bpy.types.IDOverrideLibrary.rst%60%0D%0ABlender+Version%3A+%604.5%60%0D%0A%5BPermanent+Link%5D%28https%3A%2F%2Fdocs.blender.org%2Fapi%2F4.5%2Fbpy.types.IDOverrideLibrary.html%29%0D%0A%0D%0A%2A%2AShort+description+of+error%2A%2A%0D%0A%5BPlease+fill+out+a+short+description+of+the+error+here%5D%0D%0A)
