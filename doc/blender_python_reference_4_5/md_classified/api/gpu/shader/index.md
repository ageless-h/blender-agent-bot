...
# GPU Shader Utilities (gpu.shader)[¶](#module-gpu.shader "Link to this heading")
...
Its value must be modified using the [`gpu.matrix`](../matrix/index.md#module-gpu.matrix "gpu.matrix") module.
...
gpu.shader.create\_from\_info(*shader\_info*)[¶](#gpu.shader.create_from_info "Link to this definition")
:   Create shader from a GPUShaderCreateInfo.

    Parameters:
    :   **shader\_info** ([`gpu.types.GPUShaderCreateInfo`](../types/index.md#gpu.types.GPUShaderCreateInfo "gpu.types.GPUShaderCreateInfo")) – GPUShaderCreateInfo

    Returns:
    :   Shader object corresponding to the given name.

    Return type:
    :   [`gpu.types.GPUShader`](../types/index.md#gpu.types.GPUShader "gpu.types.GPUShader")

gpu.shader.from\_builtin(*shader\_name*, *config='DEFAULT'*)[¶](#gpu.shader.from_builtin "Link to this definition")
:   Shaders that are embedded in the blender internal code (see [Built-in shaders](#built-in-shaders)).
    They all read the uniform `mat4 ModelViewProjectionMatrix`,
    which can be edited by the [`gpu.matrix`](../matrix/index.md#module-gpu.matrix "gpu.matrix") module.

    You can also choose a shader configuration that uses clip\_planes by setting the `CLIPPED` value to the config parameter. Note that in this case you also need to manually set the value of `mat4 ModelMatrix`.

    Parameters:
    :   * **shader\_name** (*str*) – One of the builtin shader names.
        * **config** (*str*) –
...
gpu.shader.unbind()[¶](#gpu.shader.unbind "Link to this definition")
...
* [Report issue
  on this page](https://projects.blender.org/blender/blender/issues/new?template=.gitea/issue_template/api_docs.yaml&field:body=%2A%2APage+Information%2A%2A%0D%0AFile%3A+%60gpu.shader.rst%60%0D%0ABlender+Version%3A+%604.5%60%0D%0A%5BPermanent+Link%5D%28https%3A%2F%2Fdocs.blender.org%2Fapi%2F4.5%2Fgpu.shader.html%29%0D%0A%0D%0A%2A%2AShort+description+of+error%2A%2A%0D%0A%5BPlease+fill+out+a+short+description+of+the+error+here%5D%0D%0A)

On this page

* GPU Shader Utilities (gpu.shader)
  + [`create_from_info()`](#gpu.shader.create_from_info)
  + [`from_builtin()`](#gpu.shader.from_builtin)
  + [`unbind()`](#gpu.shader.unbind)